...
│   ├── services/                # lógica de negócio (RN01-RN12), orquestra repositórios
│   ├── solver/                  # ⚠️ ISOLADO — nunca importa FastAPI/routers
│   │   ├── preprocessamento.py  # poda de domínio antes da geração de variáveis
│   │   ├── builder.py           # gera variáveis esparsas
│   │   ├── constraints_hard.py  # RN01, RN02, RN03, RN05, RN06
│   │   ├── constraints_soft.py  # RN04, RN08, RN12 + função objetivo
│   │   ├── prioridade_docente.py # RN12 — fórmula de prioridade docente
│   │   ├── heuristica_inicial.py # warm-start (ordenado por RN12)
│   │   ├── orquestrador_turnos.py # decomposição por turno (Manhã→Tarde→Noite)
...
| RN12 | Prioridade docente: classificação (50%) + vínculo_casa (30%) + escassez de disponibilidade (20%) | Lógica de priorização |
